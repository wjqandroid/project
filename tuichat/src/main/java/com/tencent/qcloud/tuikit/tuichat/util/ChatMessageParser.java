...
public class ChatMessageParser {
    ...
    private static TUIMessageBean parseCallingMessage(V2TIMMessage v2TIMMessage) {
        ...
        if (signalingInfo != null) { // 信令消息
            ...
            if (TextUtils.equals(businessId, TUIConstants.TUICalling.CUSTOM_MESSAGE_BUSINESS_ID) || Math.abs(businessIdForTimeout - TUIConstants.TUICalling.CALL_TIMEOUT_BUSINESS_ID) < 0.000001) {
//                return getCallingMessage(v2TIMMessage);
                //去除用户音视频沟通记录（此处只用到聊天，无需展示音视频够记录）
                return null;
            } else {
                return null;
            }
