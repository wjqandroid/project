...
public class ChatView extends LinearLayout  implements IChatLayout {
            ...
            private void startRecording() {
                post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        mRecordingTips.setText(TUIChatService.getAppContext().getString(R.string.down_cancle_send));//手指上滑，取消发送
                    }
                });
            }
                ...
                post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (status == RECORD_TOO_SHORT) {
                            mRecordingTips.setText(TUIChatService.getAppContext().getString(R.string.say_time_short));//说话时间太短
                        } else {
                            mRecordingTips.setText(TUIChatService.getAppContext().getString(R.string.record_fail));//录音失败
                        }
                    }
                });
                postDelayed(new Runnable() {
    ...
    private void showForwardDialog(boolean isMultiSelect) {
        ...
        if(messageInfoList == null || messageInfoList.isEmpty()){
            ToastUtil.toastShortMessage(getContext().getString(R.string.forward_tip));//请选择要操作的消息
            return;
        }

        if(checkFailedMessageInfos(messageInfoList)){
            ToastUtil.toastShortMessage(getContext().getString(R.string.forward_failed_tip));//发送失败消息不支持转发
            return;
        }
    ...
    private void showForwardLimitDialog(final Dialog bottomDialog, final List<TUIMessageBean> messageInfoList) {
        TUIKitDialog tipsDialog = new TUIKitDialog(getContext())
                ...
                .setTitle(getContext().getString(R.string.forward_oneByOne_limit_number_tip))//转发消息过多，暂不支持逐条转发
