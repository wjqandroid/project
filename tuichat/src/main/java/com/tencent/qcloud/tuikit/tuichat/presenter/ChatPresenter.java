...
public abstract class ChatPresenter {
    ...
    protected void addMessage(TUIMessageBean messageInfo) {
        ...
        if (messageInfo != null) {
            ...
            Log.i("TAG", "消息管理类" + "ChatPresenter+收到对方消息：" + messageInfo.toString());
            //EventBus 发布事件：收到消息成功
            EventBus.getDefault().post(new MessageEvent("receivedSuccess"));
