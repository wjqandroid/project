 ...
 * 文档 https://cloud.tencent.com/document/product/647/53538#oninvitationcancelled
 */
public class TRTCCallingImpl extends TRTCCalling {
    ...
    private V2TIMSignalingListener mTIMSignallingListener = new V2TIMSignalingListener() {
        ...
        public void onReceiveNewInvitation(String inviteID, String inviter, String groupID, List<String> inviteeList, String data) {
            ...
            //onReceiveNewInvitation inviteID:7aff43c07d97829903203b6c55cba9b3, inviter:mhsptrunkshow10214, groupID:, inviteeList:[mhsptrunkshow10307] data:{"businessID":"av_call","call_type":1,"data":{"cmd":"audioCall","room_id":1935480409,"userIDs":["mhsptrunkshow10307"]},"platform":"Android","room_id":1935480409,"version":4}
            initUserInfo();
            ...
            if (!isAppRunningForeground(mContext)) {
                TRTCLogger.d(TAG, "isAppRunningForeground is false");//应用程序是否正在运行前景为false
                return;
            }
            if (null != inviteeList && !inviteeList.contains(mCurUserId)) {
                TRTCLogger.d(TAG, "this invitation is not for me");//这个邀请不是给我的
                return;
            }

            //过滤被邀请电话
            //A呼叫B  B不在线未接听来电    B再次登录收到上次的被邀请来电
            if (filterCall()) {
                //过程邀请
...
//            ToastUtils.showShort("onInviteeAccepted+受邀者接受了"+"邀请ID:"+ inviteID+"被邀请者:"+ invitee+"数据:"+data);
            TRTCLogger.d(TAG, "onInviteeAccepted inviteID:" + inviteID + ", invitee:" + invitee + " data:" + data);
            //视频
            //onInviteeAccepted inviteID:811f2e6df5d384028aed5baef41fb56a, invitee:12608 data:{"businessID":"av_call","call_type":2,"platform":"Android","room_id":1193099583,"version":4}
            //语音
            //onInviteeAccepted inviteID:fb74efe5e4fcc93cea7e0b3859ad94c1, invitee:12608 data:{"businessID":"av_call","call_type":1,"platform":"Android","room_id":2020899100,"version":4}
...
//            ToastUtils.showShort("onInviteeRejected+受邀者拒绝了"+"被邀请者:"+ invitee+"数据:"+data);
            TRTCLogger.d(TAG, "onInviteeRejected inviteID:" + inviteID + ", invitee:" + invitee + " data:" + data);
            //语音
            //onInviteeRejected inviteID:112b9e6a5ae2bd629b099669d28e11a9, invitee:12608 data:{"businessID":"av_call","call_type":1,"platform":"Android","room_id":1352946129,"version":4}
            initUserInfo();
            SignallingData signallingData = convert2CallingData(data);
            if (!isCallingData(signallingData)) {//判断新 旧版信令
                TRTCLogger.d(TAG, "this is not the calling sense ");
                return;
            }
            if (isSwitchAudioData(signallingData)) {//切换为语音通话
                String message = getSwitchAudioRejectMessage(signallingData);
                onSwitchToAudio(false, message);
                return;
            }
//            if (!TextUtils.isEmpty(mCurCallID) && mCurCallID.equals(inviteID)) {//是否首次邀请
            mCurInvitedList.remove(invitee);
            mCurRoomRemoteUserSet.remove(invitee);// 当前语音通话中的远端用户  被邀请者
            ...
            preExitRoom(null);//用于判断是否需要结束本次通话
            stopDialingMusic();// 挂断音效
            unregisterSensorEventListener();//取消注册传感器事件监听器
//            }
        }
...
//            TRTCLogger.d(TAG, "onInvitationCancelled inviteID:" + inviteID + " data:" + data);
            //inviteID  邀请ID
            //onInvitationCancelled inviteID:74e8bd6cdbf48b2fcb2152b2ea20414d data:{"businessID":"av_call","call_type":1,"platform":"Android","room_id":882008262,"version":4}
            initUserInfo();
            SignallingData signallingData = convert2CallingData(data);
            if (!isCallingData(signallingData)) {
                TRTCLogger.d(TAG, "this is not the calling sense ");
...
//            ToastUtils.showShort("onInvitationTimeout+邀请超时"+"邀请ID:"+ inviteID);
    ...
    private boolean filterCall() {
        //服务结束时间
        SpUtil spUtil = new SpUtil(mContext, "APP启动时间");
        String app_start_time = spUtil.getString("app_start_time", "");
        long start_time = TimeFormatUtils.strToDateLong(app_start_time).getTime();
        long currentTime = Calendar.getInstance().getTime().getTime();
        if (currentTime > (start_time + 3000)) {
            return true;
        } else {
            return false;
        }
    ...
    private void handleNewSignallingInvite(SignallingData signallingData, CallModel callModel, String inviter) {
        SignallingData.DataInfo dataInfo = signallingData.getData();
        if (dataInfo == null) {
            TRTCLogger.i(TAG, "signallingData dataInfo is null");//信令数据数据信息为空
            return;
        }
    ...
    public void setSelfProfile(String userName, String avatarURL, final TRTCCallingCallback.ActionCallback callback) {
        ...
        V2TIMManager.getInstance().setSelfInfo(v2TIMUserFullInfo, new V2TIMCallback() {
            ...
            public void onError(int code, String desc) {
                TRTCLogger.i(TAG, "设置用户信息失败 code:" + code + " msg:" + desc);
                if (callback != null) {
        ...
        for (String receiver : invitedList) {
            ...
            V2TIMManager.getMessageManager().sendMessage(message, receiver, null, V2TIMMessage.V2TIM_PRIORITY_DEFAULT,
                    true, v2TIMOfflinePushInfo, new V2TIMSendCallback<V2TIMMessage>() {
                        ...
                        @Override
                        public void onError(int code, String desc) {
                            TRTCLogger.e(TAG, "sendOnlineMessage failed, code:" + code + ", desc:" + desc);//发送在线消息失败
                        }
