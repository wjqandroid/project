...
import com.visionvera.psychologist.c.module.counselling.view.ConvertCodeCenterPopup;
import com.visionvera.psychologist.c.module.counselling.view.DiscountTicketBottomPopup;
...
/**
 * @author: 刘传政
 * @date: 2020/9/21 13:25
 * QQ:1052374416
 * 作用:咨询订单提交与支付页面
 * 注意事项:
 */
public class OrderPayActivity2 extends BaseMVPActivity<OrderConsultContract.OrderPayActivity.NewOrderConsultView, OrderPayPresenter> {

    @Autowired(name = ARouterConstant.Account.accountModuleSetvice)
    IAccountService accountService;
    @BindView(R2.id.tv_title)
    TextView tv_title;
    @BindView(R2.id.recyclerViewFujian)
    ...
    private ConvertCodeCenterPopup mConvertCodePop = null;
    private DiscountTicketBottomPopup mDiscountTicketBottomPopup = null;
    ...
    public static void startActivity(Context context, IntentBean intentBean) {
        context.startActivity(new Intent(context, OrderPayActivity2.class).putExtra(Constant.IntentKey.IntentBean, intentBean));
    }

    @Override
    protected int getLayoutId() {
        return R.layout.evaluation_module_activity_order_consult_pay2;
    }
                ...
                switch (resultType) {
                    ...
                    case SERVER_NORMAL_DATAYES:
                       ...
                       /* ToastUtils.showLong(getString(R.string.evaluation_module_commit_success));
                        ...
                        tvSubmit.setText("支付");
                        showPayTypePop(OrderPayActivity2.this, mPayTypeCheckPosition);
                        break;
                    default:
                }
            }
    ...
    @OnClick({R2.id.tv_submit, R2.id.rl_back, R2.id.rlConvertCode, R2.id.ll_discount_ticket})
    public void onClick(View view) {
        ...
        if (viewId == R.id.tv_submit) {
            ...
            finish();
        } else if (viewId == R.id.rlConvertCode) {
            showConvertCodePop(OrderPayActivity2.this);
        } else if (viewId == R.id.ll_discount_ticket) {
            showDiscountTicketPop(OrderPayActivity2.this);
        }
    }
    ...
    private void http_submit() {
        ...
        NewOrderConsultRequest request = new NewOrderConsultRequest(
                ...
                mIntentBean.id,
                mIntentBean.zixunTimeId,
                0,
                0,
                23,
                mIntentBean.zixunType,
                2,
                mIntentBean.id,
                "问题类型"
        );
        request.setUploadId(uploadIds);
        request.setComments(mIntentBean.beizhu);
...
//                    net_weixinPrePay();
    ...
    private void showConvertCodePop(Context context) {
        if (mConvertCodePop != null && mConvertCodePop.isShow()) {
            return;
        }
        mConvertCodePop = (ConvertCodeCenterPopup) new XPopup.Builder(context).asCustom(new ConvertCodeCenterPopup(context));
        mConvertCodePop.show();
    }
     ...
     * @param context
     */
    private void showDiscountTicketPop(Context context) {
        if (mDiscountTicketBottomPopup != null && mDiscountTicketBottomPopup.isShow()) {
            return;
        }
        mDiscountTicketBottomPopup = (DiscountTicketBottomPopup) new XPopup.Builder(context).asCustom(new DiscountTicketBottomPopup(context));
        mDiscountTicketBottomPopup.show();
    }
