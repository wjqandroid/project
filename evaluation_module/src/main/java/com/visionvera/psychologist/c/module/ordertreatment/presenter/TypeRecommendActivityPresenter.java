...
public class TypeRecommendActivityPresenter extends BasePresenter<IContract.TypeRecommendActivity.View> implements IContract.TypeRecommendActivity.Presenter {

    EvaluationModuleModel.OrderTreatment model;

    public TypeRecommendActivityPresenter(Context context, IContract.TypeRecommendActivity.View mView) {
        super(context, mView);
    ...
    @Override
    public void getTagList(TagListRequestBean requestBean, LifecycleProvider lifecycleProvider) {
        model.getTagList(requestBean, lifecycleProvider, new Observer<TagListResponseBean>() {
            @Override
            public void onSubscribe(Disposable d) {
            ...
            @Override
            public void onNext(TagListResponseBean responseBean) {
                if (mView == null) {
                    //说明view已经销毁了,没必要再去显示了.也防止了空指针.
                    return;
                }
                if (responseBean.getCode() == Constant.NetCode.success2) {
                    if (responseBean.getResult() == null) {
                        mView.onGetTagList(responseBean, IBaseView.ResultType.SERVER_NORMAL_DATANO, context.getString(R.string.base_module_data_null));
                    } else {
                        mView.onGetTagList(responseBean, IBaseView.ResultType.SERVER_NORMAL_DATAYES, "");
                    }
                } else {
                    mView.onGetTagList(responseBean, IBaseView.ResultType.SERVER_ERROR, responseBean.getErrMsg());
                }
            }
            ...
            public void onError(Throwable e) {
                ...
                mView.onGetTagList(null, IBaseView.ResultType.NET_ERROR, e.toString());
            }
