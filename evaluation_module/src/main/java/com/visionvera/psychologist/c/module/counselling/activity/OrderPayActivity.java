 ...
 * @Version：v1.0
 * @describe： 描述:预约咨询————咨询订单提交与支付页面
 */

//支付流程：    提交订单按钮——调用完善信息接口——调用上传全部图片接口——调用支付接口

public class OrderPayActivity extends BaseMVPActivity<OrderConsultContract.OrderPayActivity.NewOrderConsultView, OrderPayPresenter> {
    ...
    protected int getLayoutId() {
        return R.layout.evaluation_module_activity_order_consult_pay;
    }
    @Override
                    ...
                    case SERVER_NORMAL_DATAYES:
                       ...
                       /* ToastUtils.showLong(getString(R.string.evaluation_module_commit_success));
                        ...
                        if (mIntentBean.price == 0) {
                            ...
                            showPayTypePop(OrderPayActivity.this, mPayTypeCheckPosition);
                        }
                        break;
                    default:
                }
...
//        @POST("gateway/counselingapi/consult/insertConsultInfo")
        NewOrderConsultRequest request = new NewOrderConsultRequest(
                ...
                mIntentBean.id, //咨询师id
                mIntentBean.zixunTimeId,//	日程ID（调取日程排班返回的ID）
                0,//申请方机构ID
                0,//申请方会议室ID
                23,//业务类型  16诊疗    23心理咨询
                mIntentBean.zixunType, //预约咨询类型    4文字咨询   5语音咨询   6视频咨询
                2, //'预约申请单来源:1网格员（B端）|2个人用户(C端)',
                mIntentBean.id, //	被预约人ID（同医生ID userId）
...
//                    net_weixinPrePay();
    ...
    private void startSuccessActivity(String payment_type) {
        ...
        intentBean.fromType = "consulting";
        OrderConsultSuccessActivity.startActivity(activity, intentBean);
...
//        request.sign = "7A45B7D080A541513B37577937C90574";
    ...
    public static class IntentBean implements Serializable {
        ...
        public int consultQuestionType;
        @Override
