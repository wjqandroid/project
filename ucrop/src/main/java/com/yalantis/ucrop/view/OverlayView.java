...
public class OverlayView extends View {
    ...
    public void setTargetAspectRatio(final float targetAspectRatio) {
        if (oldRate == 0) {
            ...
            oldRate = targetAspectRatio;
        } else {

            ValueAnimator valueAnimator = ValueAnimator.ofFloat(oldRate, targetAspectRatio);
            valueAnimator.setInterpolator(new LinearInterpolator());
            valueAnimator.setDuration(300);
            valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    float currentRate = (float) animation.getAnimatedValue();
                    mTargetAspectRatio = currentRate;
                    if (mThisWidth > 0) {
                        setupCropBounds();
                        postInvalidate();
                    } else {
